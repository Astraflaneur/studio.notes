  ...
  function makeCard(post) {
    ...
    `;
    el.querySelector('.read-more').addEventListener('click', e => {
      e.preventDefault();
      openModal(post);
    });
    return el;
  ...
  document.querySelectorAll('nav a[data-page]').forEach(a => a.addEventListener('click', e => {
    ...
    if (page === 'compose') {
      alert('Composer is available in the full editor build. For now use the "Write" flow we added earlier.');
      return;
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    let canvas = null, ctx = null, drawingInit = false;
    ...
    function ensureInitDrawing() {
      ...
      let drawing = false, points = [], undoStack = [], maxUndo = 12;
      ...
      function pointerMove(e) {
        ...
        const p = getPointerPos(e);
        points.push(p);
        drawSmoothLine(points.slice(-8), brush);
      }
  ...
  document.getElementById('downloadText').addEventListener('click', async function () {
    ...
    const outW = 1400 * DPR, outH = 700 * DPR;
    ...
    let fontSize = Math.floor(72 * DPR);
  ...
  // Gallery: extract first images from posts (if present)
  (function renderGallery() {
    const g = document.getElementById('sideGallery');
    if (!g) return;
    g.innerHTML = '';
